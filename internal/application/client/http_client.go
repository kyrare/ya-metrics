...
// NewHTTPClient конструктор для клиента
func NewHTTPClient(serverAddr string, addKey bool, workersCount uint64, cryptoKey string, logger zap.SugaredLogger) (*HTTPClient, error) {
...
func (c *HTTPClient) Send(data []metrics.Metrics) {
	c.jobs <- data
}
...
func (c *HTTPClient) newWorker(jobs <-chan []metrics.Metrics) {
...
func (c *HTTPClient) send(data []metrics.Metrics) error {
